...
import muffin.input.{AppResponse, CommandContext, DialogContext, RawAction, Router}
import org.http4s.*
...
object Http4sRoute {
  def routes[F[_], R, To[_], From[_]](router: Router[F, R], codec: CodecSupport[R, To, From])(using C: Concurrent[F]): HttpRoutes[F] = HttpRoutes.of[F] {
      ...
      handleEvent[F, UrlForm](req) { form =>
        for {
          context <- (
            ...
            form.get("user_name").get(0)).mapN[CommandContext] {
            ...
            case None => C.raiseError[CommandContext](new Exception(s"Cannot parse command ${form.toString}"))
          }
  ...
  private def handleEvent[F[_] : Concurrent, In](request: Request[F])(fun: In => F[AppResponse])(using decoder: EntityDecoder[F, In], encoder: Encode[AppResponse]): F[Response[F]] =
    for {
  ...
  private given[F[_] : Concurrent, T](using decoder: Decode[T]): EntityDecoder[F, T] = new EntityDecoder[F, T] {
